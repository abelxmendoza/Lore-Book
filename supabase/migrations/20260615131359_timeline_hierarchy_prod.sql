-- Mirrored from supabase_migrations.schema_migrations (version 20260615131359).
-- Applied on remote before this file existed in the repo.

CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

CREATE TABLE IF NOT EXISTS public.timeline_mythos (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL, title TEXT NOT NULL, description TEXT,
...
CREATE TABLE IF NOT EXISTS public.timeline_epochs (
  ...
  parent_id UUID REFERENCES public.timeline_mythos(id) ON DELETE CASCADE, title TEXT NOT NULL, description TEXT,
  start_date TIMESTAMPTZ NOT NULL, end_date TIMESTAMPTZ, tags TEXT[] DEFAULT '{}',
  source_type TEXT NOT NULL DEFAULT 'manual' CHECK (source_type IN ('import','manual','ai')),
  metadata JSONB DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW());
...
CREATE TABLE IF NOT EXISTS public.timeline_eras (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL,
  parent_id UUID REFERENCES public.timeline_epochs(id) ON DELETE CASCADE, title TEXT NOT NULL, description TEXT,
  start_date TIMESTAMPTZ NOT NULL, end_date TIMESTAMPTZ, tags TEXT[] DEFAULT '{}',
  source_type TEXT NOT NULL DEFAULT 'manual' CHECK (source_type IN ('import','manual','ai')),
  metadata JSONB DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW());
...
CREATE TABLE IF NOT EXISTS public.timeline_sagas (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL,
  parent_id UUID REFERENCES public.timeline_eras(id) ON DELETE CASCADE, title TEXT NOT NULL, description TEXT,
  start_date TIMESTAMPTZ NOT NULL, end_date TIMESTAMPTZ, tags TEXT[] DEFAULT '{}',
  source_type TEXT NOT NULL DEFAULT 'manual' CHECK (source_type IN ('import','manual','ai')),
  metadata JSONB DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW());
...
CREATE TABLE IF NOT EXISTS public.timeline_arcs (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL,
  parent_id UUID REFERENCES public.timeline_sagas(id) ON DELETE CASCADE, title TEXT NOT NULL, description TEXT,
  start_date TIMESTAMPTZ NOT NULL, end_date TIMESTAMPTZ, tags TEXT[] DEFAULT '{}',
  source_type TEXT NOT NULL DEFAULT 'manual' CHECK (source_type IN ('import','manual','ai')),
  metadata JSONB DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW());
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='chapters' AND column_name='parent_id') THEN
...
CREATE TABLE IF NOT EXISTS public.timeline_scenes (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL,
  parent_id UUID REFERENCES public.chapters(id) ON DELETE CASCADE, title TEXT NOT NULL, description TEXT,
  start_date TIMESTAMPTZ NOT NULL, end_date TIMESTAMPTZ, tags TEXT[] DEFAULT '{}',
  source_type TEXT NOT NULL DEFAULT 'manual' CHECK (source_type IN ('import','manual','ai')),
  metadata JSONB DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW());
...
CREATE TABLE IF NOT EXISTS public.timeline_actions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL,
  parent_id UUID REFERENCES public.timeline_scenes(id) ON DELETE CASCADE, title TEXT NOT NULL, description TEXT,
  start_date TIMESTAMPTZ NOT NULL, end_date TIMESTAMPTZ, tags TEXT[] DEFAULT '{}',
  source_type TEXT NOT NULL DEFAULT 'manual' CHECK (source_type IN ('import','manual','ai')),
  metadata JSONB DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW());
...
CREATE TABLE IF NOT EXISTS public.timeline_microactions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(), user_id UUID NOT NULL,
  parent_id UUID REFERENCES public.timeline_actions(id) ON DELETE CASCADE, title TEXT NOT NULL, description TEXT,
  start_date TIMESTAMPTZ NOT NULL, end_date TIMESTAMPTZ, tags TEXT[] DEFAULT '{}',
  source_type TEXT NOT NULL DEFAULT 'manual' CHECK (source_type IN ('import','manual','ai')),
  metadata JSONB DEFAULT '{}'::jsonb, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW());
...
CREATE TABLE IF NOT EXISTS public.timeline_search_index (
  ...
  layer_id UUID NOT NULL, search_text TEXT NOT NULL, tags TEXT[] DEFAULT '{}', created_at TIMESTAMPTZ DEFAULT NOW());
...
DROP TRIGGER IF EXISTS update_timeline_mythos_updated_at ON public.timeline_mythos;
...
DROP TRIGGER IF EXISTS update_timeline_epochs_updated_at ON public.timeline_epochs;
...
DROP TRIGGER IF EXISTS update_timeline_eras_updated_at ON public.timeline_eras;
...
DROP TRIGGER IF EXISTS update_timeline_sagas_updated_at ON public.timeline_sagas;
...
DROP TRIGGER IF EXISTS update_timeline_arcs_updated_at ON public.timeline_arcs;
...
DROP TRIGGER IF EXISTS update_timeline_scenes_updated_at ON public.timeline_scenes;
...
DROP TRIGGER IF EXISTS update_timeline_actions_updated_at ON public.timeline_actions;
...
DROP TRIGGER IF EXISTS update_timeline_microactions_updated_at ON public.timeline_microactions;
