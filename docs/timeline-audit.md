...
| **Character timeline** | Per-person canonical events | `resolved_events.people[]` → CanonicalTemporalModel → stitched projection | `GET /api/conversation/characters/:id/timelines` | CharacterStoryPanel | **Active** | Historical `character_timeline_events` DROPPED | **Canonical projection** |
