#!/usr/bin/env tsx
/**
 * Sprint AL — Backfill romantic relationship scores (replaces 0.5 defaults)
 */
...
async function main() {
  ...
  console.log(`Scoring relationships for user ${userId}...`);
  const { scored } = await scoreAllRelationshipsForUser(userId);
  console.log(`Done. Scored ${scored} relationship(s).`);
}
