...
describe('getLivingBiographyCard', () => {
  ...
  it('dedupes key people and filters wrong-domain interests from the home card', async () => {
    const bio = makeBio({
      facts: {
        ...
        relationships: [
          ...
          { name: 'Shana', type: 'friend', status: 'active', characterId: 'c1', relationshipId: 'r1', sourceMemoryIds: ['m1'] },
          { name: 'Shana', type: 'friend', status: 'active', characterId: 'c2', relationshipId: 'r2', sourceMemoryIds: ['m1', 'm2'] },
    ...
    expect(card.keyPeople.map(p => p.name)).toEqual(['Shana', 'Renna']);
