...
async function auditUser(userId: string) {
  ...
  const { data: timeline } = await supabaseAdmin
    ...
    .select('title,start_time,type,metadata')
    .eq('user_id', userId)
    .order('start_time', { ascending: false })
    .limit(50);
  console.log('\nTIMELINE EVENTS sample', (timeline ?? []).slice(0, 15).map((e) => ({ title: e.title, date: e.start_time, type: e.type })));
...
export async function runLifeReconstructionAudit(argv: string[] = []): Promise<void> {
  const userIds = requireUserIds(argv, ['TARGET_USER_ID'], 'Required: TARGET_USER_ID (or --user-id <uuid>)');
