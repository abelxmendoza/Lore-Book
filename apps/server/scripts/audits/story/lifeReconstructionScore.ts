...
export async function runLifeReconstructionScoreAudit(argv: string[] = []): Promise<void> {
  const [userId] = requireUserIds(
    argv,
    ['RECOVERY_USER_ID', 'TARGET_USER_ID'],
    'Required: RECOVERY_USER_ID or TARGET_USER_ID (or --user-id <uuid>)',
  );
  ...
  const threadCont = await threadContinuityScore(userId);
