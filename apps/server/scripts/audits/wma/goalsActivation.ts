...
async function inventory(userId: string) {
  ...
  const { data: bySource } = await supabaseAdmin.from('goals').select('source').eq('user_id', userId);
...
export async function traceGoalQuestion(userId: string, question: string) {
  ...
  const scoring = scoreContext(
    { foundationRecallBlock: (rag as { foundationRecallBlock?: string }).foundationRecallBlock ?? '' } as Record<
      string,
      unknown
    >,
    question,
  ...
  return {
    ...
    scoringPreservesWm: Boolean(
      (scoring.filteredLoreData as { foundationRecallBlock?: string }).foundationRecallBlock?.includes('**Goals**'),
    ),
  };
}

export async function runGoalsActivationAudit(): Promise<void> {
  ...
  console.log(
    `By status: active=${inv.stats.active_goals} completed=${inv.stats.completed_goals} abandoned=${inv.stats.abandoned_goals}`,
  );
  ...
  for (const q of GOAL_QUESTIONS) {
    const t = await traceGoalQuestion(userId, q);
    console.log(`\nQ: ${q}`);
