 ...
 *
 * The hard case is two people sharing a name (e.g. an uncle and a scene friend
 * both called "Juan"-style names). Resolution never guesses between candidates
 * that share a surface form: it disambiguates from relationship-role words and
 * alias context near the mention, honors persisted separation constraints
...
export function buildSeparationConstraints(
  ...
  // Corrections like "X's name is Juan. Do not confuse him with my tío Juan."
  // reinforce separation between every pair of same-surface entities named in
  // that record.
  ...
  for (const record of records) {
    ...
    for (const [surface, list] of aliasIndex) {
      ...
      if (!new RegExp(`\\b${escapeRegExp(surface)}\\b`, 'i').test(lower)) continue;
      ...
      const scored = candidates
        .map((candidate) => {
          ...
          for (const alias of entity.aliases) {
            ...
            if ((aliasIndex.get(a) ?? []).length === 1 && new RegExp(`\\b${escapeRegExp(a)}\\b`, 'i').test(record.text)) {
              score += 3;
            }
          }
