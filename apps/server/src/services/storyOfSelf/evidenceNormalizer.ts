...
export function classifyRecordType(text: string): NarrativeRecordType {
  ...
  if (IDENTITY_FACT_RE.test(text) && !firstPersonActionIsDominant(text)) return 'identity_fact';
...
/** "I earned my degree in 2019" is identity even though "earned" is an event verb. */
function firstPersonActionIsDominant(text: string): boolean {
  // Identity facts stated as past events keep identity typing when they
  // describe durable attributes; only treat as an event when it reads like a
  // dated episode ("last week", explicit recent framing).
...
function isThirdPartyAttribute(text: string): boolean {
  ...
  for (const s of sentences) {
    ...
    if (
      subjectMatch &&
      !/^(I|We|My|Our|The|A|An|It|This|That|There|Today|Yesterday)$/.test(subjectMatch[1]) &&
