...
class PhotoService {
  ...
  async uploadPhoto(
    ...
    options: { generateAutoEntry?: boolean } = {}
    ...
    // Generate auto journal entry from photo metadata unless the caller is
    // creating a richer vision-analysis entry itself.
    const autoEntry = options.generateAutoEntry === false
      ? undefined
      : await this.generateEntryFromPhoto(userId, url, metadata, { photoId, filename });
  ...
  /**
   * Ensure a durable photo upload appears in the Photo Album.
   * Album membership = journal entry with metadata.photoUrl / photoId.
   */
  async ensurePhotoAlbumEntry(input: {
    userId: string;
    photoUrl: string;
    photoId: string;
    filename?: string;
    ...
    if (!input.photoUrl?.trim() || !input.photoId?.trim()) {
      logger.warn({ userId: input.userId }, 'ensurePhotoAlbumEntry skipped — missing photoUrl/photoId');
      return undefined;
    }

    const content =
      (input.content && input.content.trim()) ||
      (input.filename ? `Photo uploaded: ${input.filename}` : 'Photo uploaded');
    const tags = Array.from(new Set(['photo', ...(input.tags ?? [])].filter(Boolean)));

    try {
      const captureDate = occurrenceDate(input.date);
      const entry = await memoryService.saveEntry({
        ...
        metadata: {
          ...
          albumSource: input.source ?? 'upload',
          autoGenerated: true,
          fromPhoto: true,
          ...(input.metadata ?? {}),
        },
      ...
      logger.error(
        { error, userId: input.userId, photoId: input.photoId },
        'Failed to create photo album entry',
      );
      return undefined;
    }
  }
   ...
   * Always stores photoUrl/photoId when provided so the Photo Album can show it.
   */
  async generateEntryFromPhoto(
    userId: string,
    photoUrl: string,
    metadata: PhotoMetadata,
    options: { photoId?: string; filename?: string } = {},
  ): Promise<PhotoUploadResult['autoEntry']> {
    // When we have a durable URL, always create an album row — even if metadata
    // looks "irrelevant" (screenshots, etc.). Relevance filtering only applies
    // to metadata-only sync without a stored image.
    const hasStoredPhoto = Boolean(photoUrl?.trim());
    if (!hasStoredPhoto) {
    ...
    const photoId = options.photoId || (hasStoredPhoto ? uuid() : undefined);
    ...
    try {
      ...
      let summary = options.filename
        ? `Photo uploaded: ${options.filename}`
        : 'Photo captured';
...
Generate a journal entry:`;
      ...
      if (hasStoredPhoto && photoId) {
        return this.ensurePhotoAlbumEntry({
          userId,
          photoUrl,
          photoId,
          ...
          content: summary,
          tags,
          date: photoCaptureOccurrenceDate(metadata),
          metadata: {
      ...
      // Metadata-only sync (no stored URL) — entry won't appear in album grid.
      const captureDate = photoCaptureOccurrenceDate(metadata);
      const entry = await memoryService.saveEntry({
        userId,
        content: summary,
        date: captureDate,
      ...
      // Still try a minimal album row when we have a stored URL.
      if (hasStoredPhoto && photoId) {
        return this.ensurePhotoAlbumEntry({
          userId,
          photoUrl,
          photoId,
          filename: options.filename,
          source: 'photo_metadata_fallback',
        });
      }
      return undefined;
    }
  ...
  /**
   * Generate and persist a photo memory from vision analysis. This is the path
   * for browser uploads where EXIF is often stripped, so metadata alone is not
   * enough to decide relevance or create a useful entry.
   */
  async generateEntryFromPhotoAnalysis(
    userId: string,
    input: {
      photoUrl?: string;
      photoId?: string;
    ...
    const { photoUrl, photoId, filename, metadata, analysis } = input;
    const category = analysis.category ?? 'other';
    // Still put junk uploads in the album so every stored image is visible.
    if (analysis.photoType === 'junk' && analysis.confidence >= 0.75) {
      logger.info({ filename, confidence: analysis.confidence }, 'Junk photo — creating minimal album entry');
      if (photoUrl && photoId) {
        return this.ensurePhotoAlbumEntry({
          userId,
          photoUrl,
    ...
    const detectedPeople = [
      ...
      ...(analysis.metadata?.people ?? []),
      ...(metadata.people ?? []),
    ];
    const detectedLocations = [
      ...(analysis.detectedEntities?.locations ?? []),
    ...
    const summary = (analysis.summary && analysis.summary.trim()) ||
      (detectedPeople.length > 0
        ? `Photo featuring ${Array.from(new Set(detectedPeople)).join(', ')}.`
        : `Photo uploaded: ${filename}`);

    const tags = Array.from(new Set([
      'photo',
      analysis.photoType === 'document' ? 'document' : 'memory',
      category,
      ...detectedLocations.map((loc) => loc.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')),
      ...detectedPeople.map((person) => person.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')),
    ].filter(Boolean)));

    const captureDate = photoCaptureOccurrenceDate(metadata, analysis.metadata?.date);
    const entry = await memoryService.saveEntry({
      userId,
      content: summary,
      date: captureDate,
