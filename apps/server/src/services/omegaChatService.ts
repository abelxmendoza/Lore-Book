...
import { classifyPostgresError, StorageBlockedError, throwIfStorageBlocked } from '../utils/postgresError';
...
`;
  ...
  private async persistUserMessageEarly(
    ...
    if (saveError || !savedMessage?.id) {
      ...
      if (saveError) {
        throwIfStorageBlocked(saveError);
      }
  ...
  async chatStream(
    ...
    const rawLoreData = {
      ...
      promptBreadth: (ragPacket as { promptBreadth?: string }).promptBreadth ?? 'full',
