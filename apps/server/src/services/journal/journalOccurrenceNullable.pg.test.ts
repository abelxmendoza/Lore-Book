...
import { mergeOccurrenceMetadata } from './journalOccurrenceWrite';
...
describe('nullable occurrence migration against isolated Homebrew Postgres', () => {
  ...
  it('replays pre-migration → apply → null/chronology/search proofs', async () => {
    ...
    const metadata = mergeOccurrenceMetadata(
      {},
      { mentionedAt, temporalSource: 'recording_fallback' },
    );
    expect(metadata.occurrenceUnresolved).toBe(true);
    ...
    await sql.unsafe(`
      ...
      VALUES (
        ...
        '${RECORDED_AUG}', 'unknown', NULL, $$${JSON.stringify(metadata)}$$::jsonb
      );
      INSERT INTO ${SCHEMA}.journal_entries
        (id, user_id, date, content, created_at, time_precision, "timestamp", metadata)
      VALUES (
    ...
    expect(nullRow[0].time_precision).toBe('unknown');
    expect(nullRow[0].metadata.mentionedAt).toBe(mentionedAt);
    expect(nullRow[0].metadata.occurrenceUnresolved).toBe(true);
    ...
    await sql.unsafe(`
      ...
      SET date = NULL, time_precision = 'unknown', "timestamp" = NULL
      WHERE id = '${CORRECT_ID}'
    `);
    ...
    await sql.unsafe(`
      ...
      VALUES ('${JULY_REC}', '${USER_A}', NULL, 'Unknown occurrence recorded July', '2026-07-20T12:00:00.000Z', 'unknown', NULL, '{"mentionedAt":"2026-07-20T12:00:00.000Z"}'::jsonb);
