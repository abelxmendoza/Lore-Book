...
const INTENT_RULES: Array<{ intent: WorkingMemoryIntent; pattern: RegExp }> = [
  ...
  { intent: 'SKILL_QUERY', pattern: /\b(my skills?|what skills|what skills do i have|skills (?:define|describe) me|what (?:can i do|am i good at)|what am i (?:learning|practicing|building)|my abilities|how good am i at|am i (?:improving|getting better) at|am i leveling|my proficienc|similar skills?|duplicate skills?|skills? (?:that )?(?:are )?(?:similar|related|duplicates?))\b/i },
...
const TARGET_PATTERNS = [
  ...
  /\b(?:what do you know about|what do you remember about|who is|who was|tell me about|relationship with|what happened with|between me and)\s{1,40}(.{1,120}?)[?.!]?$/i,
  /\b(?:what happened at|what went on at|what was it like at|memories at|remember at)\s{1,40}(.{1,120}?)[?.!]?$/i,
  /\b(?:how is|how's|progress on|status of)\s{1,40}(.{1,120}?)(?:\s{1,40}progressing|\s{1,40}going)?[?.!]?$/i,
