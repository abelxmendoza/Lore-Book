...
export function buildSystemPrompt(
  ...
  loreData?: {
    ...
    promptBreadth?: 'minimal' | 'focused' | 'full';
  ...
  const promptBreadth = loreData?.promptBreadth ?? 'full';
  const timelineSummary = promptBreadth === 'minimal'
    ? ''
    : orchestratorSummary.timeline.events
    .slice(0, 20)
  ...
  const rankedCharacters = promptBreadth === 'minimal' || !loreData?.allCharacters
    ? []
  ...
  const charactersKnowledge = rankedCharacters.length
    ? rankedCharacters.map((char: any) => {
        ...
        const details = [
          ...
          tier === 'FULL' && char.first_appearance
            ? `First mentioned in LoreBook (not first occurrence): ${char.first_appearance}`
            : '',
  ...
  const rankedLocations = promptBreadth === 'minimal' || !loreData?.allLocations
    ? []
  ...
  const chaptersKnowledge = promptBreadth === 'minimal' || !loreData?.allChapters?.length
    ? ''
  ...
  const timelineHierarchyKnowledge = promptBreadth === 'minimal' || !loreData?.timelineHierarchy
    ? ''
    : [
