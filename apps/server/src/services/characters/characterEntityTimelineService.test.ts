...
import { localDayKey } from '../temporal/userLocalTime';
import {
...
vi.mock('../chronologyV2/stitchedTimelineService', () => ({
  stitchedTimelineService: {
    getStitchedTimelineForEntity: vi.fn(),
  },
}));
vi.mock('../../logger', () => ({
...
describe('character entity timeline — shared projection', () => {
  ...
  it('6. ongoing state matches Omni and Calendar', () => {
    const item = stitched({
      id: 'event:ongoing',
      occurrenceStatus: 'range',
      occurredAt: '2026-08-01T00:00:00.000Z',
      occurredEnd: '2026-08-31T00:00:00.000Z',
      temporal: temporal('2026-08-01T00:00:00.000Z', {
    ...
    expect(entityModal.temporalState).toBe('ongoing');
    expect(modal.sharedExperiences[0]?.temporalState ?? modal.unresolved[0]?.temporalState).toBe('ongoing');
  });

  it('7. future state matches Omni and Calendar', () => {
    const item = stitched({
      id: 'event:planned',
      occurredAt: '2026-12-01T18:00:00.000Z',
      timePrecision: 'date',
      temporal: temporal('2026-12-01T18:00:00.000Z'),
    });
    const { omni, calendar, entityModal } = projectTemporalItemForSurfaces(item, LA, NOW);
    const modal = projectCharacterTimelineFromSources({
      entityId: JAMIE,
    ...
    expect(entityModal.temporalState).toBe('future');
    expect(modal.sharedExperiences[0]?.temporalState).toBe('future');
  });
...
describe('buildCanonicalCharacterTimeline tenant isolation', () => {
  it('loads stitched timeline by user and character only', async () => {
    vi.mocked(stitchedTimelineService.getStitchedTimelineForEntity).mockResolvedValue({
      scope_type: 'global',
    ...
    expect(stitchedTimelineService.getStitchedTimelineForEntity).toHaveBeenCalledWith('user-a', JAMIE, {
      timezone: LA,
    });
