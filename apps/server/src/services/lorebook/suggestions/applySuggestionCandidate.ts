...
export async function decideSuggestionCandidate(
  ...
  const result = (
    ...
    degraded: extra.degraded ?? (diag.decision === 'DEGRADED' || ctx.status === 'degraded' || ctx.decisionStatus === 'degraded'),
    reason: extra.reason ?? diag.reason,
    attach: diag,
    userDecision: extra.userDecision ?? userDecision,
  });

  if (attach.decision === 'DEGRADED' || ctx.status === 'degraded' || ctx.decisionStatus === 'degraded') {
    if (writePolicy !== 'user') {
      logger.debug({ extractor: input.extractor, candidate: name }, 'suggestion write degraded — no spawn');
      const reason =
        ctx.decisionStatus === 'degraded' && ctx.status !== 'degraded'
          ? 'decision_index_degraded'
          : 'canonical_index_degraded';
      return result('DEGRADED', { degraded: true, reason });
    }
  }
