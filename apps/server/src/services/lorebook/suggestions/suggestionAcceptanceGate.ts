...
import { evaluateAttachEligibility, isAttachPlan } from './suggestionAttachEligibility';
...
export function evaluateSuggestionAcceptance(input: SuggestionAcceptanceInput): SuggestionAcceptanceResult {
  ...
  if (isAttachPlan(attach) || attach.decision === 'ATTACH_EXACT' || attach.decision === 'ATTACH_ALIAS') {
    return {
      ...base,
      accept: false,
      decision: 'POSSIBLE_DUPLICATE',
      duplicateOf: attach.canonical?.name,
      confidenceAfter: calibrateConfidence(confidenceBefore, quality, { duplicate: true }),
      reason: attach.reason,
    };
  }

  if (attach.decision === 'REJECT' || attach.decision === 'DEGRADED') {
    return {
      ...base,
      accept: false,
      decision: attach.decision === 'DEGRADED' ? 'NEEDS_CONTEXT' : 'REJECTED_NOISE',
      placeRole: input.domain === 'locations' ? classifyInstitutionPlaceRole(name, evidence) : undefined,
      confidenceAfter: 0,
      reason: attach.reason,
    };
  }

  if (input.domain === 'characters' && namedOrg?.organizationType === 'software') {
    return {
      ...base,
      accept: false,
      decision: 'REJECTED_NOISE',
      confidenceAfter: 0,
      reason: 'canonical_type_conflict:software',
    };
  }
