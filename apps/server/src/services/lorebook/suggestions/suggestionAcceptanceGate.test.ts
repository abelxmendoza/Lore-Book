...
describe('suggestionAcceptanceGate — synthetic contamination corpus', () => {
  ...
  it('does not promote her friend into Characters', () => {
  ...
  it('allows an explicit visit as a Place projection without treating third-party school as a visit', () => {
    expect(classifyInstitutionPlaceRole('USC', 'I went to USC yesterday.')).toBe('protagonist_visit');
    expect(classifyInstitutionPlaceRole('USC', 'Priya graduated from USC.')).toBe('third_party');

    const visit = evaluateSuggestionAcceptance({
      name: 'Harbor Pier',
      domain: 'locations',
      evidence: 'I went to Harbor Pier yesterday.',
    });
    ...
    expect(third.placeRole === 'third_party' || third.decision === 'ROUTED' || third.decision === 'REJECTED_NOISE').toBe(true);
  });
  ...
  it('classifies a mixed conversation without cross-book contamination', () => {
    ...
    expect(evaluateSuggestionAcceptance({ name: 'USC', domain: 'locations', evidence: 'Priya graduated from USC.' }).accept).toBe(
      false,
    );
