...
describe('suggestion attach-or-alias', () => {
  ...
  it('4+19. repeated acronym rescan is idempotent', () => {
    const canon = org({
      ...
      aliases: ['USC'],
      canonicalType: 'university',
      evidence: [{ quote: 'Xingpeng graduated from USC', sourceMessageId: 'msg-1' }],
      mentionCount: 2,
    });
    const result = evaluateAttachEligibility({
      name: 'USC',
  ...
  it('11. skill synonym/normalization attaches', () => {
    const result = evaluateAttachEligibility({
      ...
      domain: 'skills',
      evidence: 'I write Python programming at Vanguard Robotics',
      userId: 'user-a',
      canon: { skills: [skill({ id: 'sk-1', name: 'Python' })] },
    });
    expect(result.decision).toBe('ATTACH_ALIAS');
    expect(result.matchBasis).toBe('normalized_skill_identity');
    if (isAttachPlan(result)) expect(result.nextAliases).toContain('Python programming');
  });
