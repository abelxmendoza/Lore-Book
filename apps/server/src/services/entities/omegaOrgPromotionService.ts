...
class OmegaOrgPromotionService {
  ...
  async promoteEntity(
    ...
    const [{ data: orgs }, { data: candidates }] = await Promise.all([
      supabaseAdmin.from('organizations').select('name, aliases').eq('user_id', userId),
      supabaseAdmin.from('group_candidates').select('id, proposed_name').eq('user_id', userId),
    ]);

    const existsAsOrg = (orgs ?? []).some((o) => {
      if (normalizeNameKey(String(o.name ?? '')) === key) return true;
      return ((o.aliases as string[] | null) ?? []).some((a) => normalizeNameKey(a) === key);
    });
    if (existsAsOrg) return null;
