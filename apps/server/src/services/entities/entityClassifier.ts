...
export function classifyEntity(name: string, context?: string): Classification {
  ...
  if (context) {
    ...
    // Checked before release cues so bare "Sol" (lover) is not swallowed by "5.6 Sol" (model)
    ...
    // Bare trailing tokens of "5.6 Sol" should not classify bare "Sol" as APP when
