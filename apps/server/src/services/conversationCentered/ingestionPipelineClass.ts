...
import { scanTemporalMentionsInTimezone, hasHistoricalDistanceCue } from '../ontology/temporalLexicon';
...
export class ConversationIngestionPipeline {
  ...
  private async extractTemporalReferences(
    ...
    for (const mention of scanTemporalMentionsInTimezone(text, now, timezone, anchorProfile)) {
    ...
    for (const mapped of collectAdditionalTemporalReferences(text, now, seenLabels, timezone)) {
      references.push({
        timestamp: clampOccurrenceDate(mapped.timestamp, now) ?? now,
        endTimestamp: mapped.endTimestamp
          ? clampOccurrenceDate(mapped.endTimestamp, now) ?? undefined
      ...
      for (let i = 0; i < utteranceIds.length; i++) {
        ...
        if (processed.splitEvents && processed.splitEvents.length > 0) {
          ...
          const types = knowledgeTypeEngineService.classifyBatch(
            units.units.map((u) => u.content || normalized.normalized_text)
          );

          // Step 6: Save extracted units
          for (let unitIndex = 0; unitIndex < units.units.length; unitIndex++) {
            const unit = units.units[unitIndex];
              ...
              for (const eventResult of assembledEvents) {
                try {
                  ...
                  if (fullEvent) {
                    ...
                    // Organization timelines (member overlap with fullEvent.people)
