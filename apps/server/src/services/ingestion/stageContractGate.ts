...
export function validateEntityCandidateBeforePersist(input: {
  ...
  if (typeUpper === 'PERSON' || typeUpper === 'CHARACTER') {
    ...
    if (bad.invalid) {
      ...
      // Retype only toward types that carry meaning (a venue, a band). A
      // software/media/process name suggested as OTHER is pollution — reject.
      if (suggested === 'LOCATION' || suggested === 'ORGANIZATION') {
