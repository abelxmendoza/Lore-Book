...
import { shouldAssembleDeterministically } from '../conversationCentered/eventAssemblyLlmGate';
...
describe('delta ingestion — event assembly LLM gate', () => {
  it('8. simple grounded event skips unnecessary LLM', () => {
    const decision = shouldAssembleDeterministically('I went to Northwind Depot with Jamie yesterday.', [
      { entity_ids: ['ent-jamie'], temporal_context: { start_time: '2026-01-01T00:00:00.000Z' }, content: 'I went to Northwind Depot with Jamie yesterday.' },
    ]);
    expect(decision.deterministic).toBe(true);
  });

  it('9. ambiguous event still uses LLM fallback', () => {
    const decision = shouldAssembleDeterministically(
      'I am not sure when Jamie and Marcus and Maya and Taylor all met before or after the trip, or maybe it was later.',
      [{ entity_ids: [], temporal_context: null, content: 'uncertain chronology' }],
    );
