...
export type ContinuityCallback = NonNullable<
  ChatStreamDoneEvent["continuityCallback"]
>;
...
export type ChatStreamResult = {
  ...
  /** Final server-selected content when post-generation verification rewrites the draft. */
  content?: string;
};
...
export const useChatStream = () => {
  ...
  const pollMemoryFeedback = useCallback(
    async (
      messageId: string,
  ...
  const streamChat = useCallback(
    async (
      ...
      /** Resume document uploaded immediately before this chat turn. */
      ...
      try {
        ...
        const useGuestStream =
          !token && getGlobalIsGuest() && guestOptions?.guestId;

        const apiUrl = config.api.url;
        const url = useGuestStream
          ? apiUrl
            ? `${apiUrl}/api/guest/stream`
            : "/api/guest/stream"
          : apiUrl
            ...
            useGuestStream
              ...
              : {
                  ...
                  ...(documentIds && documentIds.length > 0
                    ? { documentIds }
                    : {}),
                  ...(threadEntities && threadEntities.length > 0
                    ? { threadEntities }
                    : {}),
                  ...
                  ...(previewCorrections && previewCorrections.length > 0
                    ? { previewCorrections }
                    : {}),
                  ...(currentContext && currentContext.kind !== "none"
                    ? { currentContext }
                    : {}),
                  ...(soulProfileContext &&
                  (soulProfileContext.lastReferencedInsightId ||
                    (soulProfileContext.lastSurfacedInsights?.length ?? 0) > 0)
                    ? { soulProfileContext }
                    : {}),
        ...
        if (!response.ok) {
          ...
          const extractDurability = (
            body: Record<string, unknown> | null,
          ): ChatStreamDurability | undefined => {
            ...
            const noticeSaved =
              ...
              notice.code.startsWith("message_saved");
            if (
              body.userMessage ||
              body.ingestion ||
              ...
              return {
                userMessage:
                  userMessageRecord ??
                  (noticeSaved ? { persisted: true } : undefined),
                assistantResponse:
                  body.assistantResponse as ChatStreamDurability["assistantResponse"],
          ...
          if (response.status === 503) {
            ...
            if (
              ...
              userMessage =
                ...
                "Database schema incomplete. Run migrations: ./scripts/run-base-migrations.sh";
            } else {
          ...
          for (const line of lines) {
            ...
            if (data.type === "metadata") {
              ...
              if (typeof meta.messageId === "string")
                capturedMessageId = meta.messageId;
        ...
        const failureDetails =
          error && typeof error === "object" && "failureDetails" in error
            ? (error as { failureDetails?: ChatStreamFailureDetails })
                .failureDetails
            : undefined;
        onError(
