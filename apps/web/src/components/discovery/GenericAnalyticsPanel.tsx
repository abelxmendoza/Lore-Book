...
interface GenericAnalyticsPanelProps {
  moduleKey: string;
}

export const GenericAnalyticsPanel = ({ moduleKey }: GenericAnalyticsPanelProps) => {
  const analyticsModule = getModuleByKey(moduleKey);
  const { data, loading, error, refetch } = useAnalytics(moduleKey);

  if (!analyticsModule) {
    return (
      <EmptyState
        title="Module Not Found"
        description={`The analytics module "${moduleKey}" does not exist.`}
      />
    );
  }
