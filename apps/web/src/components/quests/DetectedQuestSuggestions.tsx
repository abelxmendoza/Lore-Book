...
export const DetectedQuestSuggestions = ({ onQuestAdded }: Props) => {
  ...
  const handleAdd = async (s: QuestSuggestion) => {
    ...
    try {
      ...
      onQuestAdded?.();
      // Let the board + suggestions refresh themselves.
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 sm:gap-3">
              {visible.map(s => {
                ...
                return (
                  <div
                    key={k}
                    className="relative rounded-lg border border-white/10 bg-black/40 p-3 hover:border-primary/40 transition-colors"
                  >
                    <button
                      type="button"
                    ...
                    {s.description && (
                      <p className="text-xs text-white/60 leading-relaxed line-clamp-2 mb-1.5">{s.description}</p>
                    )}
                    {s.reasoning && (
                      <p className="text-[11px] text-white/40 italic leading-relaxed line-clamp-2 mb-2">
                        {s.reasoning}
                      </p>
                    )}
