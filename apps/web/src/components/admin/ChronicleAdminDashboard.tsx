...
function StageBadge({ stage, progress }: { stage: string; progress: number }) {
  return (
    <div className="rounded-xl border border-primary/30 bg-primary/10 p-5">
      <p className="text-[10px] uppercase tracking-widest text-primary/70 mb-2">Current Stage</p>
      <div className="flex flex-wrap items-end gap-3 mb-4">
        <span className="text-2xl font-bold text-primary tracking-wide">{stage}</span>
      ...
      <p className="text-xs text-white/40 mt-2">AI updates automatically from git, docs, and project evidence.</p>
    </div>
  );
}
...
export const ChronicleAdminDashboard = () => {
  ...
  return (
    <div className="space-y-8 chronicle-admin">
      {/* Header strip */}
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
        <div>
          <p className="text-xs text-white/40 flex items-center gap-1.5">
            <BookOpen className="h-3.5 w-3.5" />
            The autobiography of LoreBook itself
          </p>
        ...
        <button
          ...
          onClick={() => load(true)}
          disabled={refreshing}
          className="flex items-center justify-center gap-2 px-4 py-2 rounded-xl border border-primary/30 bg-primary/10 text-sm text-primary hover:bg-primary/20 transition disabled:opacity-50"
        >
            ...
            {chronicle.pendingDetections.map((d) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-white/45 mt-0.5">{d.summary}</p>
                ...
                <div className="flex gap-2 shrink-0">
                  <button
                    ...
                    className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs hover:bg-emerald-500/30 transition disabled:opacity-50"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-white/5 border border-white/15 text-white/60 text-xs hover:bg-white/10 transition disabled:opacity-50"
                  >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <StageBadge stage={chronicle.stage.current} progress={chronicle.stage.progressPercent} />
          ...
          <div className="space-y-3">
            {chronicle.visionEvolution.map((v) => (
              <div key={v.id} className="border-l-2 border-primary/40 pl-3">
                ...
                <p className="text-sm text-white/75 mt-0.5">{v.vision}</p>
                <p className="text-[10px] text-white/30 mt-1">
                ...
                <ul className="space-y-3 border-l border-white/10 ml-2 pl-4">
                  {items.map((m) => (
                    <li key={m.id} className="relative">
                      ...
                      <p className="text-xs text-white/45 mt-0.5">{m.summary}</p>
      ...
      {/* Self narrative */}
      <section className="rounded-xl border border-primary/20 bg-gradient-to-br from-primary/5 to-violet-500/5 p-5">
        <h2 className="text-lg font-bold text-white mb-1">{chronicle.selfNarrative.title}</h2>
        <p className="text-sm text-white/50 mb-5">{chronicle.selfNarrative.subtitle}</p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {chronicle.selfNarrative.chapters.map((ch) => (
            <article key={ch.chapterNumber} className="rounded-lg border border-white/10 bg-black/30 p-4">
              ...
              <p className="text-sm text-white/65 leading-relaxed">{ch.body}</p>
            </article>
          ))}
        </div>
      </section>
