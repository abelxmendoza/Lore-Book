...
export const AutopilotPanel = () => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 rounded-lg sm:rounded-2xl border border-purple-900/60 bg-gradient-to-r from-[#0b0016] via-[#120028] to-[#1a0024] p-4 shadow-[0_0_30px_rgba(147,51,234,0.25)]">
        <div>
          <p className="text-xs uppercase tracking-wide text-fuchsia-200">Autopilot Engine</p>
          <h2 className="text-lg sm:text-xl font-semibold text-white">AI Life Guidance</h2>
          <p className="text-xs sm:text-sm text-white/60">Pulling insights, arcs, identity patterns, and tasks into actionable guidance.</p>
        </div>
        <Button variant="secondary" onClick={() => refresh()} disabled={loading} leftIcon={<RefreshCw className="h-4 w-4" />} className="w-full sm:w-auto">
          Refresh
        </Button>
      </div>
