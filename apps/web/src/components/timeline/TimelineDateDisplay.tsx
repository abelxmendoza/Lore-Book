...
export function TimelineDateHeader({
  ...
  const uncertainty = occurrenceUncertaintyLabel(dateKey, precision, confidence);
  const primary = precision === 'year'
    ? parsed.year
    : precision === 'month'
      ? parsed.monthShort
      : parsed.day;
  const secondary = precision === 'year'
    ? 'YEAR'
    : precision === 'month'
      ? String(parsed.year)
      : parsed.monthShort;
...
export function TimelineInlineDate({
  ...
  return (
    <div className="flex items-center gap-2 shrink-0">
      ...
      >
        <span className={`${daySize} font-bold text-white leading-none tabular-nums`}>{primary}</span>
      ...
      {(size === 'lg' || size === 'md') && (
        <div className="min-w-0 hidden sm:block">
          <p className="text-sm font-bold text-white leading-tight">{uncertainty ?? formatEventDateShort(iso)}</p>
