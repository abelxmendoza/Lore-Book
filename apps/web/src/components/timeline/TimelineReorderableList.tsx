...
export const TimelineReorderableList = ({
  ...
  const toolbar = (
    <div className="timeline-chronology__toolbar">
      <p className="timeline-chronology__hint">
        {reorderMode
          ? 'Drag or use arrows to fix order · saves help LoreBook learn your chronology'
          : 'Tap a moment to open it · highlight text to copy'}
      </p>
  ...
  if (items.length === 0) {
    return (
      <div className="timeline-chronology" data-testid="timeline-chronology">
        {toolbar}
        <div className="timeline-chronology__empty">
          <p className="timeline-chronology__empty-title">Nothing on your chronology yet</p>
          <p className="timeline-chronology__empty-body">
            When you talk about your life — or post a moment — it shows up here in time order.
          </p>
        </div>
      </div>
    );
    ...
    <div className="timeline-chronology" data-testid="timeline-chronology">
      ...
      <ol className="timeline-chronology__list">
        {items.map((item, index) => {
          ...
          return (
            <li key={item.id} className="timeline-chronology__item">
              {showYear && (
                <div className="timeline-chronology__year">
                  <span className="timeline-chronology__year-label">{year}</span>
                  <span className="timeline-chronology__year-rule" aria-hidden="true" />
                </div>
              )}
              {showDateHeader && (
                <TimelineDateHeader
                  dateKey={dateKey}
                  sticky={false}
                  className="timeline-chronology__day mx-0"
                  precision={item.timePrecision}
                  confidence={item.timeConfidence}
                />
              )}
              <div className="timeline-chronology__row">
                <span className="timeline-chronology__node" aria-hidden="true">
                  <span
                    className={`timeline-chronology__node-dot${
                      isEvent ? ' timeline-chronology__node-dot--event' : ''
                    }`}
                  />
                </span>
                <div
                  ...
                  className={[
                    'timeline-chronology__card',
                    isEvent ? 'timeline-chronology__card--event' : '',
                    selected ? 'timeline-chronology__card--selected' : '',
                    dragIndex === index ? 'timeline-chronology__card--dragging' : '',
                  ].filter(Boolean).join(' ')}
                >
                  <span className="timeline-chronology__card-accent" aria-hidden="true" />
                  <button
                    type="button"
                    onClick={() => handleCardClick(item)}
                    className={`timeline-chronology__card-body ${
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="timeline-chronology__title">{item.title}</p>
                      {item.body && item.body !== item.title && (
                        <p className="timeline-chronology__body">{item.body}</p>
                      )}
                    </div>
                  </button>
