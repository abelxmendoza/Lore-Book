...
export const CharacterDetailModal = ({
    ...
    if (activeTab === 'insights') {
      ...
      setTimeout(() => {
        ...
        const generatedInsights = {
          ...
          lastInteraction: (characterQuery?.sections?.timelines?.summary as { lastInteractionAt?: string | null } | undefined)?.lastInteractionAt
            ?? null,
          insights: [
  ...
  }, [activeTab, insights, loadingInsights, editedCharacter, characterQuery?.sections?.timelines?.summary]);

  useEffect(() => {
    const last = (characterQuery?.sections?.timelines?.summary as { lastInteractionAt?: string | null } | undefined)
      ?.lastInteractionAt ?? null;
    if (!insights || insights.lastInteraction === last) return;
    setInsights((prev: { lastInteraction?: string | null } | null) => (
      prev ? { ...prev, lastInteraction: last } : prev
    ));
  }, [characterQuery?.sections?.timelines?.summary, insights]);
