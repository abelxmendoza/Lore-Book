...
export const fetchTimelines = (filters?: {
  ...
  return fetchJson<{ timelines: Timeline[] }>(`/api/timeline?${params.toString()}`);
};

export const fetchTimeline = (id: string) =>
  fetchJson<{ timeline: Timeline }>(`/api/timeline/${id}`);

export const createTimeline = (payload: CreateTimelinePayload) =>
  fetchJson<{ timeline: Timeline }>('/api/timeline', {
    method: 'POST',
...
export const deleteTimeline = (id: string) =>
  fetchJson<void>(`/api/timeline/${id}`, { method: 'DELETE' });

// Timeline Memberships
export const addMemoryToTimeline = (timelineId: string, payload: CreateMembershipPayload) =>
  fetchJson<{ membership: TimelineMembership }>(`/api/timeline/${timelineId}/memberships`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload)
  });
...
export const fetchTimelineMemberships = (timelineId: string, limit?: number) => {
  ...
  return fetchJson<{ memberships: TimelineMembership[] }>(
    `/api/timeline/${timelineId}/memberships?${params.toString()}`
  );
};
...
export const fetchTimelineEntries = (timelineId: string, limit?: number) => {
  ...
  return fetchJson<{ entries: Array<Record<string, string | number | boolean | null> & { membership?: TimelineMembership }> }>(
    `/api/timeline/${timelineId}/entries?${params.toString()}`
  );
};

export const removeMemoryFromTimeline = (timelineId: string, entryId: string) =>
  fetchJson<void>(`/api/timeline/${timelineId}/memberships/${entryId}`, { method: 'DELETE' });
...
export const searchTimelines = (query: string, mode: SearchMode = 'natural', filters?: SearchFilters) => {
  ...
  return fetchJson<TimelineSearchResult>(`/api/timeline/search?${params.toString()}`);
};

// Timeline Relationships
export const fetchRelatedTimelines = (timelineId: string) =>
  fetchJson<{ relationships: TimelineRelationship[] }>(`/api/timeline/${timelineId}/related`);

export const createTimelineRelationship = (timelineId: string, payload: CreateRelationshipPayload) =>
  fetchJson<{ relationship: TimelineRelationship }>(`/api/timeline/${timelineId}/relationships`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(payload)
  });
