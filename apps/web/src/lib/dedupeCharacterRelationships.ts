 ...
 * Typed kinship (cousin_of, parent_of, …) wins over generic `family` /
 * `related_to`, and bidirectional edges only surface once.
 */
...
/** Higher = more specific / more useful as the primary label. */
...
function preferRelationship<T extends DedupeableRelationship>(a: T, b: T): T {
  ...
  // Prefer keeping a non-generic type label if scores tied.
...
/**
 * One row per related person. Falls back to normalized name when character_id
 * is missing so nameless duplicates still collapse.
 */
