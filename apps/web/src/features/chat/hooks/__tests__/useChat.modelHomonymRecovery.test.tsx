...
vi.mock('react-router-dom', () => ({
  useNavigate: () => vi.fn(),
  useParams: () => ({ threadId: 'thread-fixture-1' }),
}));
...
vi.mock('../../../../lib/supabase', () => ({
  useAuth: () => ({ user: { id: 'user-fixture-1' } }),
}));
...
describe('useChat — model-homonym fixture composer recovery', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    resetStorySafetyVaultForTests();
    mockHydrateThreadMessages.mockResolvedValue({ id: 'thread-fixture-1', messages: [] } as never);
    mockGetThread.mockReturnValue({ id: 'thread-fixture-1', messages: [] });
  ...
  it('does NOT restore composer when durability says the message was saved', async () => {
    ...
    expect(latestRecoverableStory('user-fixture-1', 'thread-fixture-1')).toBeNull();
  ...
  it('DOES restore composer when save truly failed', async () => {
    ...
    expect(latestRecoverableStory('user-fixture-1', 'thread-fixture-1')?.text).toBe(FIXTURE);
  });
});
