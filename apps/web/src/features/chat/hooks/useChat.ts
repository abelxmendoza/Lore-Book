...
import {
  shouldSimulateChat,
  shouldUseMockData,
} from "../../../hooks/useShouldUseMockData";
...
type LoadingStage =
  "analyzing" | "searching" | "connecting" | "reasoning" | "generating";
...
export type ChatSendOptions = {
  ...
  /** Resume document already uploaded for this turn; keeps upload and chat grounded together. */
  resumeDocumentId?: string;
  /** User-file IDs explicitly selected from the Documents book for this turn. */
  documentIds?: string[];
  /**
...
function friendlyErrorMessage(errMsg: string): string {
  ...
  if (isOpenAIBudgetExceeded(errMsg)) {
    return errMsg.includes("LoreBook hit")
      ? errMsg
...
function getDemoResponse(message: string): string {
  ...
  if (/villain|character|person|who is|tell me about/.test(lower)) {
    return (
      prefix +
      "I'd extract and track that character from your message — start the backend or sign up for the full experience with persistent memory."
    );
  }
  if (/remember|recall|what do you know|what did i|have i ever/.test(lower)) {
    return (
      prefix +
      "I'd search your stored memories and surface matching entries. Connect to the server or create an account to enable real recall."
    );
  }
  if (/log this|save this|remember this|journal entry|lore note/.test(lower)) {
    return (
      prefix +
      "Got it — this would be saved to your temporary guest lore and available in Characters and Timeline. Start the backend to try it live."
    );
  }
  return (
    prefix +
    "Your message was received. Start the backend (`npm run dev:server`) to try guest chat with real extraction, or sign up for the hosted version."
  );
}
...
export const useChat = () => {
  ...
  const { isGuest, canSendChatMessage, incrementChatMessage, guestState } =
    useGuest();
  ...
  const { success: showSuccessToast, ToastContainer: GroupToastContainer } =
    useToast({ maxVisible: 1 });
  ...
  useEffect(() => {
    // A source list belongs to one active thread/answer. Do not carry source
    // disclosures across navigation, including when the URL changes before
    // the thread adapter finishes hydrating.
    setSources([]);
  }, [urlThreadId]);
  ...
  useEffect(() => {
    ...
    mutateThreadMessagesForThread(threadId, (prev) => {
      ...
      const recoveredUser: Message = {
        ...
        lifecycle: withLifecyclePatch(
          createPendingLifecycle(attempt.createdAt),
          {
        ...
        metadata: {
          ...
          ...(attempt.resumeDocumentId
            ? { resumeDocumentId: attempt.resumeDocumentId }
            : {}),
          ...(attempt.documentIds?.length
      ...
      const notice: Message = {
        ...
        role: "assistant",
        content:
          "Cloud sync failed. Draft is on this device — Retry sync without rewriting.",
        timestamp: new Date(),
        ...
        messages.map((message) =>
          message.id === streamId
            ? {
                ...message,
                isStreaming: false,
                persistStatus: message.content?.trim()
                  ? (message.persistStatus ?? "pending")
  ...
  const sendMessage = useCallback(
    async (messageText: string, options?: ChatSendOptions) => {
      const hasImages = (options?.images?.length ?? 0) > 0;
      const hasResume = Boolean(options?.resumeDocumentId);
      const hasDocuments = (options?.documentIds?.length ?? 0) > 0;
      const resolvedText =
        messageText.trim() ||
        (hasImages
          ? IMAGE_ATTACHED_PLACEHOLDER
          : hasResume
            ? "I attached a resume to this chat."
            : hasDocuments
              ? "I attached documents to this chat."
              : "");
      if (
        (!resolvedText && !hasImages && !hasResume && !hasDocuments) ||
        sendLockRef.current ||
        loading
      )
        return;
      // Sources describe the answer currently being generated, not the whole
      // thread. Clear them before every new turn to prevent stale provenance.
      setSources([]);
      ...
      const postNotice = (message: Message) => {
        if (noticeThreadId) {
          mutateThreadMessagesForThread(noticeThreadId, (prev) => [
            ...prev,
            message,
          ]);
          return;
      ...
      const safetyAttempt: StorySafetyAttempt = {
        ...
        text: resolvedText,
        createdAt:
          recovered?.id === clientIdempotencyKey
            ? recovered.createdAt
            : new Date().toISOString(),
        ...(options?.resumeDocumentId
          ? { resumeDocumentId: options.resumeDocumentId }
          : {}),
      ...
      if (retry) {
        const existing =
          getThread(threadId)?.messages.find(
            (m) => m.id === retry.existingUserMessageId,
        ...
        userMessage = {
          ...
          lifecycle: withLifecyclePatch(existing.lifecycle, {
            ...
            cloudPersistence:
              retry.mode === "assistant_response" ? "saved" : "syncing",
            processing: "queued",
          ...
          ...(localOk
            ...
            : {
                lastError: {
                  ...
                  message:
                    "Could not write a durable device copy (storage full or blocked).",
                  retryable: true,
                  occurredAt: new Date().toISOString(),
                },
              }),
      ...
      const assistantMessageId =
        retry?.noticeMessageId ?? `assistant-${Date.now()}`;
      if (retry?.noticeMessageId) {
        ...
        mutateThreadMessagesForThread(streamThreadId, (prev) => [
          ...prev,
          assistantMessage,
        ]);
      }
      ...
      const composerThread = (options?.composerEntities ?? [])
        ...
        .filter(
          (
            ...
            id: string;
            name: string;
            type: "character" | "location" | "organization" | "skill";
          } => e !== null,
        );
      ...
      const mergedThreadEntities = mergeThreadEntities(
        baseThread,
        composerThread,
      );
      ...
      const applyPersistence = (persistence?: PersistPayload) => {
        ...
        if (persistence.user) {
          if (persistence.user.saved && persistence.user.id) {
            ...
            threadPersistenceTracker.markSyncFailed(
              streamThreadId,
      ...
      try {
        if (!user && isSimulatedChatRuntime()) {
          const chatMode = shouldUseMockData()
            ? ("demo" as const)
            : ("guest" as const);
        ...
        await streamChat(
          ...
          (chunk) => {
            ...
            const contentProgress = Math.min(
              25,
              (accumulatedContent.length / 1000) * 25,
            );
          ...
          (meta) => {
            ...
            if (
              (meta.groupCreated || meta.groupRenamed || meta.groupDeleted) &&
              typeof meta.organizationName === "string"
            ) {
              const verb = meta.groupDeleted
                ? "deleted"
                : meta.groupRenamed
                  ? "renamed"
                  : "created";
              showSuccessToast(
                `Group "${meta.organizationName}" ${verb}.`,
                5000,
                "group",
              );
            }
            ...
            if (
              meta.reclassifiedTo &&
              typeof meta.reclassifiedSourceName === "string"
            ) {
              const book =
              ...
              showSuccessToast(
                `Moved "${meta.reclassifiedSourceName}" to ${book}${merge}.`,
                5000,
                "reclassify",
              );
              window.dispatchEvent(
                new CustomEvent("lk:locations-updated", {
                  detail: {
                    ids: [meta.reclassifiedSourceId],
                    deleted: meta.reclassifiedFrom === "location",
                  },
                }),
              );
            }
            ...
            if (
              meta.locationWriteOperation === "delete" &&
              typeof meta.locationName === "string"
            ) {
              showSuccessToast(
                `Place "${meta.locationName}" deleted.`,
                4000,
                "location",
              );
              window.dispatchEvent(
              ...
              meta.locationWriteOperation &&
              typeof meta.locationName === "string"
            ) {
              showSuccessToast(
                `Place "${meta.locationName}" ${meta.locationWriteOperation === "create" ? "saved" : "updated"}.`,
                4000,
                "location",
              );
              window.dispatchEvent(
              ...
              setSources((prev) => {
                ...
                for (const source of meta.sources) {
                  if (
                    ...
                    !source.id ||
                    !source.title?.trim() ||
                    /^(?:assistant|system|lorebook response|this answer)$/i.test(
                      source.title.trim(),
                    )
                  )
                    continue;
          ...
          (result) => {
            ...
            const assistantCloudSaved = Boolean(
              persistedAssistantMessageId ||
              metadata?.persistence?.assistant?.saved,
            );
            updateStreamMessage(
              assistantMessageId,
              {
                ...
                isStreaming: false,
                persistStatus: assistantCloudSaved
                  ? "saved"
                  : metadata?.persistence?.assistant?.saved === false
                    ? "failed"
                    : "pending",
                lifecycle: withLifecyclePatch(undefined, {
                  localPersistence: "saved",
            ...
            const doRefresh = () => {
              apiCache.deletePattern(
                /\/api\/(entries|timeline|chapters|characters|entity-resolution|family-trees|organizations)/,
              );
              Promise.all([
                refreshEntries(),
                refreshTimeline(),
          ...
          (error, durability, generationFailure) => {
            ...
            const useDemoFallback =
              ...
              (isGuest &&
                (isBackendUnavailable(error) || isGuestStreamBlocked(error))) ||
            ...
            if (userSaved) {
              const savedId =
                durability?.userMessage?.id ?? persistedUserMessageId;
              updateStreamMessage(userMessage.id, {
                ...(savedId ? { id: savedId } : {}),
                persistStatus: "saved",
                lifecycle: withLifecyclePatch(userMessage.lifecycle, {
                ...
                metadata: {
                  ...
                  durability,
                  ingestionStatus:
                    ingestionStatus ?? (ingestionActive ? "QUEUED" : undefined),
                  frontendRecoveryDecision,
                  generationFailure,
                },
              });
          ...
          (feedback) => {
            updateStreamMessage(assistantMessageId, {
              cognitionFeedback: feedback,
            });

            const detected = feedback.entitiesDetected ?? [];
            const newlyCreated = detected.filter(
              (e) => e.created === true && e.entityId,
            );
            const alreadyExisting = detected.filter(
              (e) => e.created === false && e.entityId,
            );
            ...
            if (alreadyExisting.length > 0) {
              ...
              const pinnedIsCharacter =
                options?.chatFocus?.entityType === "character";
        ...
        const useDemoFallback =
          (!user && isSimulatedChatRuntime()) ||
          (isGuest &&
            (isBackendUnavailable(errMsg) || isGuestStreamBlocked(errMsg))) ||
  ...
  const resolveRetryTarget = useCallback(
    (messageId: string) => {
      const threadMsgs =
        (urlThreadId ? getThread(urlThreadId)?.messages : null) ?? messages;
      ...
      if (clicked.role === "user") {
        ...
        return {
          ...
          resumeDocumentId:
            typeof clicked.metadata?.resumeDocumentId === "string"
              ? clicked.metadata.resumeDocumentId
              : undefined,
          documentIds: Array.isArray(clicked.metadata?.documentIds)
            ? clicked.metadata.documentIds.filter(
      ...
      const userMessage =
        ...
        (key
          ? threadMsgs.find(
              (m) =>
                m.role === "user" && m.metadata?.clientIdempotencyKey === key,
            )
          : null) ??
        (() => {
          const idx = threadMsgs.findIndex((m) => m.id === messageId);
          return idx > 0 && threadMsgs[idx - 1]?.role === "user"
            ? threadMsgs[idx - 1]
            : null;
        })();
      ...
      return {
        ...
        resumeDocumentId:
          typeof userMessage.metadata?.resumeDocumentId === "string"
            ? userMessage.metadata.resumeDocumentId
            : undefined,
        documentIds: Array.isArray(userMessage.metadata?.documentIds)
          ? userMessage.metadata.documentIds.filter(
              (id): id is string => typeof id === "string",
            )
          : undefined,
      };
  ...
  const retryCloudSync = useCallback(
    async (messageId: string) => {
      ...
      await sendMessage(target.originalText, {
        ...
        ...(target.resumeDocumentId
          ? { resumeDocumentId: target.resumeDocumentId }
          : {}),
        ...(target.documentIds?.length
